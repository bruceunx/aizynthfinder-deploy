from typing import Tuple, Dict, Sequence, List, Union, Optional
import atexit
import io
import shutil
import tempfile
...
#
#
def draw_rounded_rectangle(img: Image, color: PilColor, arc_size: int = 20) -> Image:
...
class RouteImageFactory:
    ...
    def __init__(
        ...
        self._stock_lookup: Dict = {}
        self._mol_lookup: Dict = {}
    ...
    def _add_effective_size(self, tree_dict: Dict) -> None:
        ...
        if children:
            tree_dict["eff_height"] = sum(
                child["eff_height"]
                for child in children) + self.margin * (len(children) - 1)
            tree_dict["eff_width"] = (max(child["eff_width"] for child in children) +
                                      tree_dict["image"].size[0] + self.margin)
        else:
    ...
    def _add_pos(self, tree_dict: Dict, pos: Tuple[int, int]) -> None:
        ...
        mid_y = pos[1] + int(
            tree_dict["image"].height * 0.5)  # Mid-point of image along y
        children_height = sum(child["eff_height"]
                              for child in children) + self.margin * (len(children) - 1)
        childen_leftmost = min(pos[0] - self.margin - child["image"].width
                               for child in children)
        ...
        for idx, (child, child_y0) in enumerate(zip(children, child_ys)):
            ...
            if not child.get("children") and idx == 0 and len(children) > 1:
                ...
                child_y = (child_ys[idx - 1] + self.margin +
                           children[idx - 1]["image"].height)
            self._add_pos(child, (child_x, child_y))

    def _extract_mol_tree(self, tree_dict: Dict) -> Dict:
        dict_ = {
        ...
        if tree_dict.get("children"):
            dict_["children"] = [
                self._extract_mol_tree(grandchild) for grandchild in tree_dict.get(
                    "children")[0]["children"]  # type: ignore
                if not (grandchild.get("hide", False) and not self.show_all)
            ]
        return dict_

    def _extract_molecules(self, tree_dict: Dict) -> None:
    ...
    def _make_image(self, tree_dict: Dict) -> None:
        ...
        self._draw.ellipse((mid_x - 8, mid_y - 8, mid_x + 8, mid_y + 8),
                           fill="black",
                           outline="black")
